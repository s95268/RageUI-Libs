...
local SettingsButton = {
    ...
    Text = { X = 15, Y = 7, Scale = 0.25 },
    LeftBadge = { Y = -2, Width = 40, Height = 40 },
    RightBadge = { X = 375, Y = 0, Width = 40, Height = 40 },
    RightText = { X = 425, Y = 7, Scale = 0.25 },
...
function RageUI.Button(Label, Description, Style, Enabled, Action, Submenu)
    ...
    if CurrentMenu ~= nil and CurrentMenu() then
        ...
        if CurrentMenu.Pagination.Minimum <= Option and CurrentMenu.Pagination.Maximum >= Option then
            ...
            local haveLeftIcon = Style.LeftIcon and Style.LeftIcon ~= RageUI.BadgeStyle.None
            ...
            RenderRectangle(CurrentMenu.X + 15, CurrentMenu.Y + SettingsButton.SelectedSprite.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, SettingsButton.SelectedSprite.Width + CurrentMenu.WidthOffset - 30, SettingsButton.SelectedSprite.Height - 3, 20,25,30, 255)
            ...
            if Active then 
                RenderRectangle(CurrentMenu.X + 15, CurrentMenu.Y + SettingsButton.SelectedSprite.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, SettingsButton.SelectedSprite.Width + CurrentMenu.WidthOffset - 427, SettingsButton.SelectedSprite.Height - 3, 0, 126, 255, 255)
            end
            ...
            if Enabled then
                ...
                if haveLeftIcon then
                    if (Style.LeftIcon ~= nil) then
                        RenderSprite('items', Style.LeftIcon, CurrentMenu.X + 25, CurrentMenu.Y + SettingsButton.LeftBadge.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset + 5, 30, 30, 0, 255, 255, 255, 255)
                    end
                end
                ...
                local LeftIconX = haveLeftIcon and 30 or 0

                RenderText(not Active and Label or Label, CurrentMenu.X + SettingsButton.Text.X + LeftBadgeOffset + 15 + LeftIconX, CurrentMenu.Y + SettingsButton.Text.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, 8, SettingsButton.Text.Scale, R_ITEM_BUTTON, G_ITEM_BUTTON, B_ITEM_BUTTON, 255);
            else
                if haveRightBadge then
